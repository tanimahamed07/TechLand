...
export const getMainCategories = async () => {
  ...
  const mainCategories = result.data.filter((cat: any) => !cat.parentCategory);
  return { ...result, data: mainCategories };
};

// ৪. নির্দিষ্ট parent এর অধীনে সব subcategories
export const getSubcategoriesByParent = async (parentId: string) => {
  const response = await fetch(BASE_URL);
  if (!response.ok) throw new Error("Failed to fetch categories");
  const result = await response.json();
  ...
  const subcategories = result.data.filter(
    (cat: any) => cat.parentCategory === parentId,
  );
